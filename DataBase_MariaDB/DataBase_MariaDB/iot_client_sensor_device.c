...
//		name_msg[str_len-1] = 0;   //'\n' 제거
		...
		else if(!strcmp(pArray[1], "TRASH"))
		{
			int height1 = atoi(pArray[2]);
			int height2 = atoi(pArray[3]);

			sprintf(sql_cmd, "insert into sensor(name, date, time, trash_height_1, trash_height_2) values('%s', now(), now(), %d, %d)",pArray[0], height1, height2);
